    ...
    if (client.available()) {
      ...
      if (c == '\n') {
        ...
        if (header.indexOf("GET /?password=") >= 0) {
          ...
          authenticated = (receivedPassword == webpagePassword);
        }
        ...
        client.println("setInterval(()=>{fetch('/distance').then(res=>res.text()).then(data=>{document.getElementById('distance').textContent=data})},200);</script>");
